...
use servo::{
    ClipboardDelegate, ContextMenu, ContextMenuAction, ContextMenuElementInformation,
...
#[derive(Default)]
struct SimpleClipboardDelegate {
    data: RefCell<String>,
}

impl SimpleClipboardDelegate {
    fn data(&self) -> String {
        self.data.borrow().clone()
    }
}

impl ClipboardDelegate for SimpleClipboardDelegate {
    fn clear(&self, _webview: WebView) {
        *self.data.borrow_mut() = String::new();
    }

    fn get_text(&self, _webview: WebView, request: StringRequest) {
        request.success(self.data.borrow_mut().clone());
    }
...
struct ContextMenuTest {
    ...
    clipboard_delegate: Rc<SimpleClipboardDelegate>,
}
...
impl ContextMenuTest {
    fn new(body: &str) -> Self {
        ...
        let clipboard_delegate = Rc::new(SimpleClipboardDelegate::default());
...
fn test_contextual_context_menu_items() {
    let image_url = r"data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==\;";
    let test = ContextMenuTest::new(&format!(
        "<!DOCTYPE html>\
        <a href=\"https://servo.org\"><div style=\"width: 50px; height: 50px;\">Link</div></a> \
        <div><img src=\"{image_url}\" style=\"width: 50px; height: 50px;\"></div> \
...
#[test]
fn test_contextual_context_menu_select_all_and_copy() {
    ...
    assert_context_menu_items(
        ...
        ContextMenuElementInformation {
            flags: ContextMenuElementInformationFlags::Selection,
            link_url: None,
            image_url: None,
        },
    );
    context_menu.select(ContextMenuAction::Copy);

    let clipboard_delegate = test.clipboard_delegate.clone();
    test.servo_test
        .spin(move || clipboard_delegate.data().is_empty());
    assert_eq!(test.clipboard_delegate.data(), "abcdef");
    test.clipboard_delegate.clear(test.webview);
}
