...
pub mod image_cache {
    ...
    use net_traits::image_cache::{
        ...
        ImageCacheResult, ImageLoadListener, PendingImageId, VectorImageId,
    };
    ...
    impl ImageCacheFactoryImpl {
        pub fn new(_broken_image_icon_data: Vec<u8>) -> Self {
            Self
        }
    }
    ...
    impl ImageCacheFactory for ImageCacheFactoryImpl {
        fn create(
            ...
            _font_resolver: Arc<dyn FontResolver>,
        ) -> Arc<dyn ImageCache> {
            Arc::new(WorkerImageCache)
        }
    }

    struct WorkerImageCache;

    impl ImageCache for WorkerImageCache {
        ...
        fn evict_rasterized_image(&self, _svg_id: &Uuid) {}
        ...
        fn add_listener(&self, _listener: ImageLoadListener) {}
        fn notify_pending_response(&self, _id: PendingImageId, _action: FetchResponseMsg) {}
        fn dispatch_fill_key_cache_with_batch_of_keys(&self, _image_keys: Vec<ImageKey>) {}
        fn clear(&self) {}
    }
}
#[cfg(not(target_arch = "wasm32"))]
