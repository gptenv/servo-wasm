...
The current ABI 8 production-stripped artifact, built with locked dependencies from mozjs-wasm `f942001b4`, is **62,916,046 bytes** (about 60.0 MiB), SHA-256 `2dd782cbb839dea9f50b82adf66e24261dc117bdc012ff98ad489f31269f3096`; the local Wrangler 4.136.3 dry run bundled 61,508.75 KiB of Worker JavaScript and WASM, below the 64 MiB limit. The Worker imports exactly five `env` functions, with no WASI or wasm-bindgen imports:
...
The Worker adapter instantiates the module, creates a Servo instance, installs the fetch and WebSocket bridges, and advances the cooperative event loop. `npm test` passes **123 tests** against the current artifact, including runaway-script termination by the script work budget (loops, event/timer handlers, generators, async/microtask/promise storms, eval, regexp backtracking and recurring timers, each followed by a successful evaluation), custom-element upgrade, MutationObserver records, repeated canceled-navigation stress, nested timer/interval coverage, IndexedDB open/write/read, ordered Cache Storage lifecycle operations, Worker storage estimates, origin isolation across navigation, and credential-aware final/redirect cookie handling. Local workerd smoke checks pass, including the root integration response, **45/45** shared fixture runs over three rounds (15 distinct fixtures), a screenshot response and a `/runaway` page whose infinite loop is terminated while the runtime keeps answering. The host timer deadline export and `pumpUntilSettled()` use Worker `scheduler.wait()` where available with a cancellable timer fallback.

The source implements checked **version-8 host ABI**. Earlier versions added Worker-host WebSocket event/action envelopes, redirect-cookie processing, Worker-pumped `requestAnimationFrame` callbacks and a narrow fetch DTO; version 7 adds the per-turn script work budget that terminates runaway synchronous scripts, and version 8 adds correlated, promise-awaiting `evaluate()`. It also has `loadHtml(html, {url})`, deterministic initial `about:blank` bootstrapping, navigation coalescing and cancellation of active/queued host fetches. Response delivery enforces header/chunk/terminal ordering. Mid-body failures reject body consumers instead of succeeding with truncated content. The original web-platform-style corpus covers templates, selectors, DOM fragments/clones, event propagation, CSS rule mutation/computed-style invalidation, shadow DOM, and microtask/timer ordering. It is not the upstream WPT runner or a claim of complete web conformance. The exact contract is in [WORKER-ABI.md](../ports/servo-js-wasm/WORKER-ABI.md).

The host adapter queues requests above six concurrent outbound connections and caps actual host `fetch()` calls (including redirect hops) at 50 per invocation by default, with a separate 10,000-request runtime-session cap. `beginInvocation()` resets only the first counter when the host starts a new serialized MCP call. The host remains responsible for Cloudflare's actual per-invocation accounting and must call this boundary explicitly when reusing a runtime.

The runtime remains cooperative and reset is not destruction: it cancels fetches and navigates to `about:blank`, but Servo and SpiderMonkey stay alive for the lifetime of the WASM instance. The current renderer uses `vello_cpu` over Servo display lists; it is not WebRender's native painter and has documented gaps (backdrop filters and non-rounded clip paths; masks are only covered by selected fixtures). Request bodies are buffered up to 256 KiB; responses stream in 64 KiB chunks with an 8 MiB default limit. Non-credentialed CORS, including Fetch preflights decided in the engine, is supported; credentialed cross-origin requests and redirects after a preflight fail closed. `document.cookie`, same-runtime request cookies and credential-permitted final/followed same-origin redirect cookies use an in-memory jar; cross-site SameSite handling and persistence across WASM instances are missing. General request streaming, complete redirect behavior, reader/clone cancellation and several browser APIs remain unsupported or uncharacterized. Runaway scripts are terminated by a per-turn work budget in the mozjs fork; it bounds interpreter work, not CPU time or memory. Current ABI details and renderer coverage live in [WORKER-ABI.md](../ports/servo-js-wasm/WORKER-ABI.md).
...
Define one documented Worker feature profile that excludes desktop shell, multiprocess, WebDriver server, devtools server, native media backends, WebGL, WebGPU, filesystem caches, and platform font discovery unless they have a real Worker implementation. DOM, HTML parsing, CSS parsing/style, SpiderMonkey, URL, and fetch are enabled. WebGL and WebGPU are the only intentional browser API exclusions: Workers do not provide a general GPU device, and the CPU display-list renderer cannot implement WebGL's rendering contract. Cookie support is partial: document script access and same-runtime request/final-response/followed same-origin redirect cookies use an in-memory jar, but full SameSite/credentialed CORS behavior and cross-instance persistence are missing.
...
`pumpStatus()` advances one cooperative turn and reports progress/fetch dispatches; `pumpUntilSettled()` applies caller budgets, waits on fetch activity and timer deadlines, and returns unsettled on budget exhaustion. It is a quiescence heuristic; the engine's per-turn script work budget, not the pump deadline, terminates runaway synchronous scripts. Nested timeout ordering and interval cancellation have production-artifact coverage; host callback/reentrancy edge coverage remains. Keep callers' finite budgets explicit.
...
The host-pull scheduler, timer IDs/cancellation, deadline export and basic promise/microtask ordering are implemented. The ABI 4 change adds a Worker-pumped refresh timer so `requestAnimationFrame` can run without a timer thread. Nested timeout order and interval cancellation are covered by the production-artifact suite. Host callback/reentrancy edge cases remain. A host deadline cannot interrupt a synchronous infinite page script, so ABI 7 terminates it with the engine's per-turn script work budget (see WORKER-ABI.md).

## 8. Workstream F — storage and other browser services

Storage remains incomplete. `localStorage` and `sessionStorage` use an in-memory per-instance service. `document.cookie`, same-runtime credentialed requests, final HTTP responses and followed same-origin redirects use Servo's in-memory RFC 6265 jar. Production-artifact tests confirm final and redirect cookie forwarding, `credentials: "omit"` behavior and `HttpOnly` script isolation. Stores disappear when the WASM instance is discarded. The `servo-mcp` host pins one runtime inside a per-session Durable Object, so in-memory stores survive separate MCP calls while that runtime stays live; a DO/runtime interruption ends the session and its storage. Cooperatively pumped client-storage, IndexedDB and Cache Storage services use in-memory SQLite/maps. Production-artifact tests cover IndexedDB open/write/read and ordered Cache Storage open/has/keys/delete. `navigator.storage.estimate()` resolves with a 32 MiB quota estimate and a registry-metadata usage lower bound; the quota is not yet enforced against endpoint writes. IndexedDB transaction ordering and callbacks still need broader verification, and Servo's Cache API surface lacks request/response operations such as `match`, `put` and `add`. Reset does not clear all instance storage; a new instance provides isolation.

Persistence target (2026-09-25): use the existing per-session [SQLite-backed Durable Object](https://developers.cloudflare.com/durable-objects/best-practices/access-durable-objects-storage/) as the authoritative store, keyed by origin and storage area (plus browsing context for sessionStorage). Keep WASM memory as a live cache and restore storage into a fresh runtime after eviction or deployment; a live SpiderMonkey heap is not a durable snapshot. Record cookies, localStorage and IndexedDB/Cache metadata through a versioned host storage bridge when operations commit, so a completed operation is not lost between MCP calls. Store large Cache API response bodies in SQLite chunks below the [2 MB Durable Object SQL row/BLOB limit](https://developers.cloudflare.com/durable-objects/platform/limits/); [R2](https://developers.cloudflare.com/r2/platform/limits/) is optional if cache size or traffic later warrants a separate object store. The matching host integration belongs to the separately owned `servo-mcp` project.

Required before the storage APIs can be called supported:

- Broaden verification of Worker-pumped IndexedDB and client-storage services for transaction ordering, callbacks, and IndexedDB origin isolation. The Worker backend uses per-instance in-memory SQLite and does not depend on native filesystem paths or storage threads; the current suite verifies IndexedDB open/write/read and localStorage/sessionStorage/Cache Storage isolation when a live instance navigates between origins.
- Complete Cache Storage request/response operations and decide whether cached bodies remain instance-local or use a host persistence interface. The current worker-pumped service covers only the CacheStorage-level name/open/delete/keys operations.
- Finish cookie coverage: followed same-origin redirects now capture `Set-Cookie`, credentials omission is enforced, and `HttpOnly` stays inaccessible to script. Cross-site SameSite context checks and other redirect modes still need work.
- Preserve the existing per-session Durable Object/runtime boundary; make sure stores cannot be shared between session IDs. Implement the versioned host storage bridge, commit operations to the session's SQLite storage, and restore browser data into a new WASM runtime after restart. The current host reports interrupted sessions because it cannot yet perform that restoration.
- Enforce a realistic quota across the in-memory endpoints and count their data in `navigator.storage.estimate()`; the current Worker quota is an unenforced 32 MiB estimate and usage counts only registry metadata. `persist()` and `persisted()` correctly return false for the ephemeral Worker backend; revise their behavior when Durable Object persistence is implemented.
- Verify cryptographic randomness remains fail-closed and CSPRNG-backed; route browser-visible wall-clock use through the Worker host bridge.

The release target intentionally excludes WebGL and WebGPU. Translating arbitrary WebGL calls into this CPU display-list renderer is not a reasonable general compatibility shim; it is a different rendering contract, and Workers do not provide a general GPU device for WebGPU. All other browser APIs remain in scope unless they are explicitly classified as unsupported with a concrete platform or security reason. The current Worker capability array is a host-port summary, not that complete inventory.
...
7. A GitHub Actions workflow builds the exact Worker profile incrementally and runs import/size checks through the Node suite plus local workerd smoke routes. Earlier runs exposed missing Servo uv setup, then missing WASI C/C++ archives and Clang 18 below SpiderMonkey's version-19 minimum. The next run reached SpiderMonkey compilation but SDK Clang could not find `stdio.h` under the raw target. The pinned SDK 29 archive puts headers under `include/wasm32-wasi`; that directory is not selected automatically for `wasm32-unknown-unknown`. The target wrapper now passes the explicit sysroot and target-specific C/C++ include directories, preserving the fork's worker include shims first. Local SDK 29 Clang C, C++, and SpiderMonkey-style `#include_next` probes pass; a green remote run is still required. The workflow installs Ubuntu Clang 19 for host tools and the checksum-verified WASI SDK 29 toolchain (Clang 21) for the target, then uses `cargo build --locked`. Preserve existing build artifacts and do not require a clean build. Existing native-target gating warnings remain, and the forked native-target changes need validation where relevant.
...
- [x] The ABI 7 production artifact builds incrementally with locked dependencies, imports exactly the five allowed `env` functions, has no WASI/wasm-bindgen imports, and stays below the size limit.
- [x] Current artifact passes the 123-test Node suite, including runaway-script termination, IndexedDB open/write/read, ordered Cache Storage lifecycle, origin isolation across navigation, storage estimates, and same-origin final/redirect cookie forwarding; local workerd root smoke, 45 shared fixture runs over three rounds (15 distinct fixtures), screenshot and runaway-script responses also pass.
- [x] Runaway synchronous scripts are terminated by an engine-side per-turn work budget (mozjs fork), since no Worker watchdog or advancing clock exists; the default is calibrated in WORKER-ABI.md against Node, and remote CPU calibration remains.
...
- [x] Upgrade evaluation beyond its serialized single-result slot: `evaluate()` (ABI 8) adds correlation IDs, promise awaiting, a structured result/error contract and timeout/cancel/reset outcomes.
...
- [ ] Get the full CI workflow green. Authenticated logs exposed missing WASI archives, an outdated Clang and a missing target-specific header path; the locally validated wrapper needs a remote run.
