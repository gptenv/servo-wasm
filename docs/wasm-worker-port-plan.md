...
The Worker adapter instantiates the module, creates a Servo instance, installs the fetch and WebSocket bridges, and advances the cooperative event loop. `npm test` passes **58 tests** against the current artifact, including custom-element upgrade, MutationObserver records, repeated canceled-navigation stress, nested timer/interval coverage, IndexedDB open/write/read, Cache Storage lifecycle operations, and final-response cookie forwarding to a same-origin fetch. Local workerd smoke checks pass, including the root integration response, **45/45** shared fixture runs over three rounds (15 distinct fixtures), and a screenshot response. The host timer deadline export and `pumpUntilSettled()` use Worker `scheduler.wait()` where available with a cancellable timer fallback.
...
Storage remains incomplete. `localStorage` and `sessionStorage` use an in-memory per-instance service. `document.cookie`, same-runtime credentialed requests and final HTTP responses use Servo's in-memory RFC 6265 jar. A production-artifact test confirms final-response cookies are sent on a subsequent same-origin fetch; redirect response cookies are not yet captured, and stores disappear when the WASM instance is discarded. The `servo-mcp` host pins one runtime inside a per-session Durable Object, so in-memory stores survive separate MCP calls while that runtime stays live; a DO/runtime interruption ends the session and its storage. Cooperatively pumped client-storage, IndexedDB and Cache Storage services use in-memory SQLite/maps. Production-artifact tests now cover IndexedDB open/write/read and Cache Storage open/has/keys/delete. IndexedDB transaction ordering and callbacks still need broader verification, and Servo's Cache API surface lacks request/response operations such as `match`, `put` and `add`. Reset does not clear all instance storage; a new instance provides isolation.

Required before the storage APIs can be called supported:

- Broaden verification of Worker-pumped IndexedDB and client-storage services for transaction ordering, callbacks, persistence across calls in one live instance, and origin isolation. The Worker backend uses per-instance in-memory SQLite and does not depend on native filesystem paths or storage threads; the current suite verifies IndexedDB open/write/read.
...
- Finish cookie coverage: capture `Set-Cookie` from redirect responses, enforce credentials-mode and same-site rules across redirects, and keep `HttpOnly` inaccessible to script.
- Preserve the existing per-session Durable Object/runtime boundary; make sure stores cannot be shared between session IDs. The current host reports interrupted sessions after a DO restart because it cannot restore a live WASM runtime from SQLite metadata.
- Set realistic quotas and define how `navigator.storage.estimate()` and persistence requests behave.
...
- [x] Current artifact passes the 58-test Node suite, including IndexedDB open/write/read, Cache Storage lifecycle, and same-origin cookie forwarding; local workerd root smoke, 45 shared fixture runs over three rounds (15 distinct fixtures), and screenshot response also pass.
