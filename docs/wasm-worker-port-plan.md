...
Storage remains incomplete. `localStorage` and `sessionStorage` use an in-memory per-instance service. `document.cookie`, same-runtime credentialed requests and final HTTP responses use Servo's in-memory RFC 6265 jar. Redirect response cookies are not captured, and these stores disappear when the WASM instance is discarded. The `servo-mcp` host pins one runtime inside a per-session Durable Object, so in-memory stores survive separate MCP calls while that runtime stays live; a DO/runtime interruption ends the session and its storage. This change adds cooperatively pumped client-storage, IndexedDB and Cache Storage services backed by in-memory SQLite/maps. IndexedDB transaction ordering and callbacks still need production-artifact verification, and Servo's Cache API surface lacks request/response operations such as `match`, `put` and `add`. Reset does not clear all instance storage; a new instance provides isolation.

Required before the storage APIs can be called supported:

- Verify the new Worker-pumped IndexedDB and client-storage services preserve transaction ordering, callbacks and origin isolation. Their Worker backend uses per-instance in-memory SQLite and does not depend on native filesystem paths or storage threads.
