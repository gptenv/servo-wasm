...
| R8 compatibility | **In progress P1.** `worker-compatibility-matrix.md` records tested paths and known partial, unsupported and unverified areas. A trap audit of reply-bearing resource messages fixed instance-killing traps in `URL.revokeObjectURL`, `new File()`/`FormData` and `pushState` traversal, and hangs in `Blob.text()` and blob-URL fetches, by adding an in-memory blob store and history-state storage. | Supported-site corpus, scoped WPT results and inventory covering workers, media, forms, streams, permissions, accessibility and renderer behavior. |
