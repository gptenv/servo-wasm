...
| R7 page operations | **Open P1.** Evaluation has one result slot and does not await promises. The 50-subrequest default is a runtime-lifetime budget. | Correlated operations, deadlines, history/redirect/stream cancellation tests and a documented split between invocation and session limits. |
| R8 compatibility | **Open P1.** `WORKER-ABI.md` and capability report name known partial and unsupported areas but are not a complete API matrix. | Supported-site corpus, scoped WPT results and inventory covering workers, media, forms, streams, permissions, accessibility and renderer behavior. |
| R9 release evidence | **In progress P1.** CI now installs a pinned WASI sysroot; it records logs, the artifact and a JSON manifest with hashes, ABI, tool versions and locked Git revisions. | Green CI at exact release commit, complete uploaded-bundle check and remote CPU/memory/latency measurements. |
| R10 forks | **Open P1/P2.** Cargo.lock records revisions; fork patch/security/license inventories and native-path validation are incomplete. | Locked release manifest, upstream baselines, patch owners, native tests, SBOM and notices for the distributed artifact. |
