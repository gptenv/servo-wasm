...
| --- | --- | --- |
| R1 execution and resources | **Open P0.** Synchronous JS can loop without an engine interrupt. Raster decoding and Worker SVG rasterization have 8 Mpixel per-image limits, but per-response caps and a storage estimate do not bound aggregate memory or temporary allocations. | SpiderMonkey interrupt/deadline design, loop/promise/decompression/image/storage stress tests, concurrent isolate memory and recovery measurements. |
...
| R6 ABI | **In progress P1.** ABI 6 uses a narrow fetch DTO instead of serializing internal `RequestBuilder` fields. The adapter checks the ABI and required redirect-cookie export at initialization; it now bounds and validates host commands with malformed-message tests. | Broader state-transition fuzzing and pairwise old/new compatibility tests. |
